...
export const ChallengeAnalysisSchema = z.object({
  summary: z
    .string()
    .describe('A crisp 1-2 sentence executive summary of the core civic problem.'),
  domain: z
  ...
  subDomain: z
    .string()
    .nullable()
    .describe('Specific sub-discipline (e.g., Rural Water Infrastructure, Soil Health Monitoring).'),
  suggestedPriority: z
  ...
  problemType: z
    .string()
    .describe('Category of issue, e.g., Infrastructure Breakdown, Diagnostic Access, Environmental Risk.'),
  ...
  suggestedApproach: z
    .array(z.string())
    .min(1)
    .describe('High-level innovation directions (e.g., IoT Telemetry, Vernacular Tech, Mobile Diagnostic).'),
  requiredExpertise: z
    .array(z.string())
    .min(1)
    .describe('Disciplines and institutions needed (e.g., Embedded Electronics, Hydro-geology, Agronomy).'),
  ...
  needsHumanReview: z
    .boolean()
    .describe('True if information was ambiguous, critical hazard flagged, or detail insufficient.'),
});
