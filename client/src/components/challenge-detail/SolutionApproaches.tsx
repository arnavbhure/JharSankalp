...
export function SolutionApproaches({ challenge, onProposeIdea }: SolutionApproachesProps) {
  return (
    <section id="solution-ideas" className="py-12 sm:py-16 border-b border-[#EEEAE1] text-left">
      ...
      {/* Solution Approaches Grid or Empty State */}
      {challenge.solutionApproaches && challenge.solutionApproaches.length > 0 ? (
        ...
        <div className="rounded-2xl border border-dashed border-[#EEEAE1] bg-white p-8 sm:p-10 text-center space-y-4 max-w-2xl mx-auto">
          ...
          <div className="space-y-1">
            <h3 className="text-[1.15rem] font-bold text-[#1D2522]">
              No Solution Approaches Proposed Yet
            </h3>
            <p className="text-[13.5px] text-[#6B5845] max-w-md mx-auto leading-relaxed">
              This challenge is currently open for exploration. Be the first innovator, researcher, or community group to propose a solution approach!
            </p>
          </div>
          <button
            onClick={onProposeIdea}
            className="inline-flex items-center gap-2 rounded-lg bg-[#123B2A] hover:bg-[#0D2B1E] text-white px-5 py-2.5 text-[13.5px] font-bold shadow-xs transition-all active:scale-[0.98] cursor-pointer"
          >
            <Lightbulb className="h-4 w-4 text-[#F5A623]" />
            <span>Submit First Solution Proposal</span>
          </button>
        </div>
      )}
    </section>
